...
const iconMap = { instagram: InstagramIcon, linkedin: LinkedInIcon }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div {...fadeUp(0.56)} className="mt-10">
          <p className="text-[10px] tracking-[0.3em] uppercase text-gold-primary/35 mb-4 font-mono">SEGUICI</p>
          <div className="flex gap-8 items-center">
            {content.contact.social.map((s) => {
              const Icon = iconMap[s.icon as keyof typeof iconMap]
              return (
                <a key={s.name} href={s.href} aria-label={s.name}
                  className="text-gold-primary/50 hover:text-gold-primary transition-colors duration-200 hover:scale-110 transform">
                  <Icon />
                </a>
              )
            })}
